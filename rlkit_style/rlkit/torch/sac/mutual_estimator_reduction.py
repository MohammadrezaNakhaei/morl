...
class MIERSoftActorCritic(OfflineMetaRLAlgorithm):
    def __init__(
            self,
            env,
            train_tasks,
        ...
        self.qf1, self.qf2, self.vf, self.c, self.mi_estimator, self.decoder = nets[1:]
        ...
        self.mi_optimizer                   = optimizer_class(self.mi_estimator.parameters(), lr=self.behavior_lr)
        self._num_steps                     = 0
    ...
    @property
    def networks(self):
        nets =  self.agent.networks + [self.qf1, self.qf2, self.vf, self.target_vf, self.c, self.mi_estimator, self.decoder]
        return nets
    ...
    def _take_step(self, indices, context):
        ...
        if self.use_mi:
            ...
            mi_estimator_loss = self.mi_estimator.learning_loss(obs_action, task_z.detach())
            mi_estimator_loss.backward()
            self.loss['mi_estimator'] = mi_estimator_loss.item()
        ...
        if self.use_mi:
            ...
            estimated_mi = self.mi_estimator(obs_action, task_z)
            mi_entropy_loss = self.mi_loss_weight*estimated_mi.mean()
        ...
        if self.use_mi:
            self.eval_statistics['train/mi_estimator_loss'] = ptu.get_numpy(mi_estimator_loss)
